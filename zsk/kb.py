...
from kb_ontology import ONTOLOGY, PRIORITY_LEVELS, PREDEFINED_TAGS, reload_ontology, save_ontology, load_ontology
from kb_ontology import get_domain, get_domain_label, is_custom_ontology, get_ontology
...
def cmd_build(kb: KnowledgeBase, args):
    ...
    # ── 2. 本体论 ──
    print("=" * 65)
    print(f"📂 本体分类体系（{len(ONTOLOGY)} 个一级分类）")
    print("=" * 65)
...
1. 阅读上方研报章节列表，为每个章节确定分类和优先级。
   - 分类来自 {len(ONTOLOGY)} 个本体类: {', '.join(ONTOLOGY.keys())}
...
def cmd_discover(kb: KnowledgeBase, args):
    """
    扫描 reports/ 所有文档，输出摘要供 Agent 阅读后提议自定义本体分类。
    Agent 阅读此输出后，提议分类 → 写入 ontology_config.json。
    """
    from kb_doc_reader import read_document, UnsupportedFormatError, list_supported_formats
    from kb_import import _list_documents

    reports_dir = PROJECT_DIR / "reports"
    doc_files = _list_documents(reports_dir)

    if not doc_files:
        supported = list_supported_formats()
        print("❌ reports/ 目录下没有文档。")
        print(f"   支持的格式: {', '.join(f for f, ok in supported.items() if ok)}")
        return
    ...
    print("=" * 65)
    print()
    if is_custom_ontology():
        print(f"📋 当前领域: {get_domain_label()} ({get_domain()})")
        print(f"   已有 {len(ONTOLOGY)} 个分类: {', '.join(ONTOLOGY.keys())}")
        print(f"   💡 将基于文档内容重新发现，覆盖现有分类。")
    else:
        print(f"📋 当前使用默认 Agent 开发分类（{len(ONTOLOGY)} 个）")
        print(f"   💡 如果文档不属于 Agent 领域，将自动发现新分类。")
    print()

    # ── 文档概览 ──
    print("=" * 65)
    print(f"📄 发现 {len(doc_files)} 份文档")
    print("=" * 65)
    ...
    for doc_file in doc_files:
        ...
        from kb_import import _parse_headings
        sections, _ = _parse_headings(text)
        suffix = doc_file.suffix.lower()

        if suffix in (".md", ".markdown") and sections:
            print(f"  格式: MD(有标题)  |  章节: {len(sections)}  |  总长度: {len(text)} 字符")
            for sec in sections[:5]:
                print(f"    {'#' * sec['level']} {sec['title']}")
            if len(sections) > 5:
                print(f"    ... 还有 {len(sections) - 5} 个章节")
        else:
            from kb_doc_reader import detect_structure
            struct = detect_structure(text)
            fmt = suffix if suffix not in (".md", ".markdown") else "MD(无标题)"
            print(f"  格式: {fmt}  |  结构: {struct.get('type', '?')}  |  总长度: {len(text)} 字符")
...
1. 阅读上方所有文档的完整内容（不仅仅是摘要/章节标题）。

2. 理解文档的领域主题。文档属于什么领域？医学？金融？法律？技术？

3. 根据文档内容，提议 5-10 个一级分类（本体分类）。
   每个分类需要：
...
4. 将分类体系输出为 JSON，然后用以下命令写入：
   python {PROJECT_DIR}/kb.py ontology-set '<JSON>'
   ...
   {{
     ...
     "categories": {{
       "cardiology": {{
         "label": "心脏病学",
         "description": "心脏疾病的诊断、治疗与预防",
         "l2": ["coronary", "arrhythmia", "heart-failure"]
       }},
       "neurology": {{
         "label": "神经病学",
         "description": "神经系统疾病的研究与治疗",
         "l2": ["stroke", "epilepsy", "neurodegenerative"]
       }}
     }}
   }}

5. 写入后，运行 kb.py build 即可基于新分类体系构建知识库。
""")


def cmd_ontology_set(kb: KnowledgeBase, args):
    """设置自定义本体分类。从命令行参数或 stdin 读取 JSON。"""
    ...
    if not data_str:
        ...
        if not sys.stdin.isatty():
            ...
            print("❌ 请提供 JSON 数据：python kb.py ontology-set '<json>'")
            print("   或通过管道: echo '<json>' | python kb.py ontology-set")
            return
    ...
    if not categories:
        print("❌ JSON 中缺少 'categories' 字段")
        return

    domain = data.get("domain", "")
    domain_label = data.get("domain_label", "")

    config_path = save_ontology(
    ...
    # 重新加载本体 + 更新本地引用
    import kb_ontology as _kbo
    reload_ontology()
    globals()["ONTOLOGY"] = _kbo.ONTOLOGY

    print(f"✅ 本体配置已保存: {config_path}")
    print(f"   领域: {domain_label} ({domain})")
    print(f"   分类: {len(ONTOLOGY)} 个")
    for cat_id, cat_info in categories.items():
        l2_count = len(cat_info.get("l2", []))
        l2_hint = f" ({l2_count} 个二级)" if l2_count else ""
        print(f"     {cat_id}: {cat_info.get('label', cat_id)}{l2_hint}")
    print()
    print(f"   💡 现在可以运行 kb.py build 或对 Agent 说「加载 zsk skill，构建知识库。」")


def cmd_ontology_show(kb: KnowledgeBase, args):
    """显示当前本体分类配置。"""
    print("=" * 50)
    print(f"📂 当前本体配置")
    print("=" * 50)
    print(f"领域: {get_domain_label()} ({get_domain()})")
    print(f"来源: {'自定义配置' if is_custom_ontology() else '默认（Agent 开发）'}")
    print(f"分类: {len(ONTOLOGY)} 个")
    print()

    for cat_id, cat_info in ONTOLOGY.items():
        l2_list = cat_info.get("l2", [])
        print(f"  {cat_id:25s} {cat_info['label']}")
        print(f"  {'':25s} {cat_info['description']}")
        if l2_list:
            print(f"  {'':25s} 二级: {', '.join(l2_list)}")
        print()

    if not is_custom_ontology():
        print("💡 提示: 运行 kb.py discover 来基于你的文档发现新领域分类。")


def cmd_ontology_reset(kb: KnowledgeBase, args):
    """重置为默认 Agent 分类。"""
    from kb_ontology import reset_to_default
    import kb_ontology as _kbo
    reset_to_default()
    reload_ontology()
    globals()["ONTOLOGY"] = _kbo.ONTOLOGY
    print("✅ 已重置为默认 Agent 开发分类。")
    print(f"   分类: {len(ONTOLOGY)} 个")
...
示例:
  ...
  python kb.py delete abc123
    ...
    # discover
    sub.add_parser("discover", help="发现文档领域，输出分析供 Agent 提议自定义本体分类")

    # ontology-set
    p = sub.add_parser("ontology-set", help="设置自定义本体分类（JSON）")
    p.add_argument("data", nargs="?", help='JSON 字符串: \'{"domain":"...","categories":{...}}\'')

    # ontology-show
    sub.add_parser("ontology-show", help="显示当前本体分类配置")

    # ontology-reset
    sub.add_parser("ontology-reset", help="重置为默认 Agent 分类")
    ...
    commands = {
        ...
        "ontology-set": cmd_ontology_set,
        "ontology-show": cmd_ontology_show,
        "ontology-reset": cmd_ontology_reset,
    }
